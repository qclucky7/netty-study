...
import com.netty.study.websocket.demo2.util.Response;
...
public class BananaWebSocketServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    private void handleWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
        ...
        try {
            ...
            if (CODE.online.code.intValue() == request.getServiceId()) { // 客户端注册
                ...
                if (Strings.isNotBlank(requestId)) {
                    response.setIsSucc(false).setMessage("requestId不能为空");
                    return;
                } else if (Strings.isNotBlank(request.getName())) {
                ...
                if (Strings.isNotBlank(requestId)) {
                    ...
                    response.setIsSucc(false).setMessage("name不能为空");
                } else if (Strings.isNotBlank(request.getMessage())) {
            ...
            LOG.warning("Json解析异常");
        } catch (Exception e2) {
            LOG.warning("处理Socket请求异常");
        }
    }
    ...
    public void sendWebSocket(String msg) throws Exception {
        ...
        this.ctx.channel().writeAndFlush(new TextWebSocketFrame(msg));
    }
